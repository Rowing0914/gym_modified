...
class MujocoEnv(gym.Env):
    ...
    def render(self, mode='human', annotation_flg=True, width=DEFAULT_SIZE, height=DEFAULT_SIZE):
        if self.viewer and annotation_flg:
            # Issue: https://github.com/openai/mujoco-py/issues/423
    ...
    def add_joints_markers(self):
        ...
        - Note:
            Changeable attributes are shown L:232 to L:248 of Reference above => (As of 31/07/2019)
        """
        for actuator_name in self.sim.model.actuator_names:
            ...
            self.viewer.add_marker(pos=actuator_pos,
                                   label="%s: %.3f" % (actuator_name, self.sim.data.ctrl[actuator_id - 1]),
